-- Transform raw API responses into raw_weather_forecasts table
-- This script extracts forecast weather records from JSONB
-- Uses fetched_at as forecast_created_at timestamp

WITH unique_forecasts AS (
    SELECT DISTINCT ON (ws.id, (weather->>'timestamp')::TIMESTAMP, r.fetched_at, (weather->>'source_id')::INTEGER)
        ws.id as station_id,
        (weather->>'timestamp')::TIMESTAMP as timestamp,
        (weather->>'source_id')::INTEGER as source_id,
        r.fetched_at as forecast_created_at,
    ...
    WHERE NOT r.processed
      AND ws.observation_type = 'forecast'
    ORDER BY ws.id, (weather->>'timestamp')::TIMESTAMP, r.fetched_at, (weather->>'source_id')::INTEGER, r.id DESC
)
...
ON CONFLICT (station_id, timestamp, forecast_created_at, source_id) DO NOTHING;
